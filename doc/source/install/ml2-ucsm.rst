...
#. Nova needs to be made aware of compute hosts that have SR-IOV capable devices.
   This is achieved by adding the ``pci_passthroug_whitelist`` configuration to
   ``nova.conf`` on each compute host capable of hosting SR-IOV based VMs.
