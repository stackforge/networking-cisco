...
Description
^^^^^^^^^^^
The configuration option `https_verify` was explicitly set to False since the
default is True.  This setting should not be used in production since the
...
indicating which switch configuration this setting was found.
...
* If the certificate and key files have not been configured on the participating
  Nexus 9K, configure them using the Nexus Management CLI `nxapi certificate`
...
* Remove 'https_verify=False' from the neutron start-up configuration.  This will
  default to verification of public certificate to occur.
