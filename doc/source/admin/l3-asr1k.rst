...
.. code-block:: console
    ...
    root@tme166 ~(keystone_admin)]# Global-router-0000-000000000003
    +-------------------------------------------------+--------------------------------------+
    | Field                                           | Value                                |
    +-------------------------------------------------+--------------------------------------+
    | admin_state_up                                  | True                                 |
...
* Hosting-device states reported by the CFGA

HD_ACTIVE = ‘ACTIVE’
Active means the hosting device is up, responds to pings and is configurable.

HD_NOT_RESPONDING = ‘NOT RESPONDING’
Not responding means the hosting device does not respond to pings but has not yet been determined to be dead or faulty.

HD_ERROR = ‘ERROR’
Error means the hosting device has been determined to be faulty; meaning it may respond to pings but other symptoms indicate it is faulty.

HD_DEAD = ‘DEAD’
Dead means the hosting device has been determined to be dead in that it does not respond to pings even given multiple, repeated attempts.
