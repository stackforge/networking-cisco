...
class HTParser(object):
    """
    A hierarchical text parser. Indents are used to derive parent child
    hierarchy.
    """
    ...
    def _build_indent_based_list(self):
        ...
        for line in self.cfg:
            ...
            if match and line.strip() is not None or line.strip()[0] != "!":
                item = (len(match.group(1)), line)
                self._indent_list.append(item)

    def _find_starts(self, linespec):
        """
        Returns the start points as list in the following format:
