...
opt_cpnr_group = cfg.OptGroup(name='cisco_pnr',
                         title='Cisco Prime Network Registrar configuration')
...
class DhcpRelayAgent(object):
    ...
    def __init__(self):
        ...
        LOG.debug('(cisco_pnr) http_server: {}'.format(self.conf.
                  cisco_pnr.http_server))
        LOG.debug('(cisco_pnr) external_interface: {}'.format(self.conf.
                  cisco_pnr.external_interface))
        LOG.debug('(cisco_pnr) dhcp_server_addr: {}'.format(self.conf.
                  cisco_pnr.dhcp_server_addr))
        LOG.debug('(cisco_pnr) dhcp_server_port: {}'.format(self.conf.
                  cisco_pnr.dhcp_server_port))
        self.ns_states = {}
