...
class RoutingServiceHelper(object):
    ...
    def _process_new_ports(self, ri, new_ports, ex_gw_port, list_port_ids_up):
        #TODO(bmelande): 1. We need to handle the case where a global router 
        #                   is attached to a secondary subnet by an update.  
        #                   Such an update does NOT result in a new router
        #                   port but rather as an update of an EXISTING port 
        #                   where a subnet is added to it. 
        #                2. We need to handle the case where the global router
        #                   is no longer attached to the primary subnet so 
        #                   that one of the global router's secondary ip 
        #                   addresses must become the new primary address.
        #                   Such an update does NOT result in removal of a 
        #                   router port but rather as an update of an EXISTING
        #                   port where a subnet is added to it.
        for p in new_ports:
            ...
            LOG.debug("Number of subnets associated with port = %d" %
                      num_subnets_on_port)
            if (ri.router['routerrole:role'] is None and num_subnets_on_port > 1):
            ...
            ### REMOVE BELOW
            #if (ri.router['routerrole:role'] == c_constants.ROUTER_ROLE_GLOBAL):
            ### END OF REMOVE
            # Configure the primary IP address
            self._set_subnet_info(p, port_subnets[0]['id'])
            self._internal_network_added(ri, p, ex_gw_port)
   
            # Process the secondary subnets. Only router ports of global routers
            # can have multiple ipv4 subnets since we connect global routers to
            # external networks using regular router ports.
    ...
    def _process_gateway_set(self, ri, ex_gw_port, list_port_ids_up):
        ...
        #TODO(bmelande): Do NOT assume there is only a single subnet. Handle 
        #                any secondary subnets too 
    ...
    def _set_subnet_info(port, subnet_id, is_primary=True):
        ...
        ips = [i['ip_address'] for i in port['fixed_ips'] if i['subnet_id'] == subnet_id] 
        if not ips:
        ...
        ### REMOVE BELOW attributes when we've confirmed the ip_info dict attributes works
        # NOTE! 'ip:cidr' is used in several .py files so we need to do a "search & replace with"
        #       ['ip_info']['ip_cidr'] before we remove it.
        port['ip_cidr'] = "%s/%s" % (ips[0], prefixlen)
        port['is_primary'] = is_primary
        ### END OF REMOVE
