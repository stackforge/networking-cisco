...
class ASR1kRoutingDriver(iosxe_driver.IosXeRoutingDriver):
    ...
    def _get_item(self, list_containing_entries_with_subnet_ids, subnet_id,
                  attribute_name='subnet_id'):
	for item in list_containing_entries_with_subnet_ids:
           if item[attribute_name] == subnet_id:
               return item
        return {}
    ...
    def _create_sub_interface(self, ri, port, is_external=False, gw_ip=""):
        ...
        if port.get('is_primary', True) is True:
...
#    @staticmethod
    def _is_port_v6(self, port):
        cidr = self._get_item(
            port['subnets'], port['ip_info']['subnet_id'], 'id')['cidr']
