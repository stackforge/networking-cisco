...
class CiscoUcsmMechanismDriver(api.MechanismDriver):
    ...
    def update_port_precommit(self, context):
        ...
        if not vlan_id:
            LOG.warn(_LW('Vlan_id is None. Ignoring this port'))
            return
        ...
        if not self.driver.check_vnic_type_and_vendor_info(vnic_type,
            ...
            sp_template = self.ucsm_config.get_sp_template_for_host(host_id)
            LOG.debug('SP Template: %s, VLAN_id: %d', str(sp_template),
                      vlan_id)
        ...
        if (self.driver.check_vnic_type_and_vendor_info(vnic_type, profile) and
            ...
            if self.ucsm_config.sp_template_mode:
                sp_template = self.ucsm_config.get_sp_template_for_host(
                    host_id)
                LOG.debug('Setting ucsm_updated flag for vlan_id: %s, '
                    'sp_template : %s on ucsm_ip: %s', vlan_id, sp_template,
                    ucsm_ip)
