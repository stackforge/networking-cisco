...
class CiscoUcsmDriver(object):
    ...
    def __init__(self):
        ...
        self.supported_pci_devs = config.UcsmConfig.parse_pci_vendor_config()
    ...
    def is_vmfex_port(self, profile):
        ...
        return (vendor_info in self.supported_pci_devs)
    ...
    def ucsm_connect_disconnect(self, hostname):
        handle = self.ucs_manager_connect(hostname)
        if not handle:
        ...
        try:
            ...
            self.ucs_manager_disconnect(handle, hostname)
    ...
    def ucs_manager_connect(self, hostname):
        ...
        ucsm_ip, ucsm_username, ucsm_password = config.get_ucsm_for_host(
            hostname)
    ...
    def _handle_ucsm_exception(self, exception_type, profile_type,
        ...
        ucsm_ip = config.get_ucsm_ip_for_host(host_id)
    ...
    def update_serviceprofile(self, host_id, vlan_id):
        ...
        """
        service_profile = config.get_service_profile_for_host(host_id)
        if service_profile:
