...
class CiscoUcsmDriver(object):
    ...
    def _create_port_profile(self, handle, profile_name, vlan_id,
        ...
        try:
            ...
            qos_policy = self.ucsm_conf.get_sriov_qos_policy()
            if not qos_policy:
                qos_policy = ""
            LOG.debug('SD : QoS Policy : %s', qos_policy)
            ...
            if trunk_vlans:
                for vlan in trunk_vlans:
                    ...
                    if not mo:
                        LOG.warning(_LW('UCS Manager network driver cannot '
                                        'associate Vlan %d to Port Profile %s'),
                            vlan, profile_name)
