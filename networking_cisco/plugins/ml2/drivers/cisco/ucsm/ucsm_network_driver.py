...
class CiscoUcsmDriver(object):
    ...
    def _ssl_cert_check(self):
        if not config.get_ucsm_https_verify():
            LOG.warning('SSL certificate verification has been disabled '
                            'for the UCSM driver. The connection to UCS '
                            'Manager(s) is now insecure.')
            try:
                _create_unverified_https_context = (
                    ssl._create_unverified_context)
            except AttributeError:
                # Python versions that don't verify HTTPS certificates
                # by default
                pass
            else:
