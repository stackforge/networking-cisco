...
class CiscoUcsmDriver(object):
    ...
    def __init__(self):
        ...
        self.supported_sriov_vif_types = [const.VIF_TYPE_802_QBH,
                                          const.VIF_TYPE_802_QBG]
    ...
    def _handle_ucsm_exception(self, exception_type, profile_type,
        ...
        if const.DUPLICATE_EXCEPTION in str(exception_type):
            ...
            # Raise a Neutron exception. Include a description of
            # the original  exception.
            ucsm_ip = self.ucsm_conf.get_ucsm_for_host(host_id)
            raise cexc.UcsmConfigFailed(config=profile_name,
                                        ucsm_ip=ucsm_ip,
                                        exc=exception_type)

    def _create_vlan_profile(self, host_id, vlan_id):
        ...
        with self.ucsm_connect_disconnect(host_id) as handle:
            try:
    ...
    def _create_port_profile(self, host_id, profile_name, vlan_id, vnic_type):
        ...
        with self.ucsm_connect_disconnect(host_id) as handle:
            try:
                ...
                if not port_profile:
                    LOG.debug('UCS Manager network driver Port Profile path '
                              'at %s missing', const.PORT_PROFILESETDN)
                    return False
                ...
                if not p_profile:
                    LOG.debug('UCS Manager network driver could not create '
                              'Port Profile %s at %s',
                              profile_name, port_profile_dest)
                    return False
                ...
                if not mo:
                    LOG.debug('UCS Manager network driver cannot associate '
                              'Vlan Profile %s to Port Profile %s', vlan_name,
                              profile_name)
                    return False
                ...
                if not cl_profile:
                    LOG.debug('UCS Manager network driver could not create '
                              'Client Profile %s at %s',
                              cl_profile_name, cl_profile_dest)
                    return False
    ...
    def create_portprofile(self, host_id, profile_name, vlan_id, vnic_type):
        ...
        """
        # Create Vlan Profile
        if not self._create_vlan_profile(host_id, vlan_id):
            LOG.error(_LE('UCS Manager network driver failed to create '
                          'Vlan Profile for vlan %s'), str(vlan_id))
            return False

        # Create Port Profile
        if not self._create_port_profile(host_id, profile_name, vlan_id,
            vnic_type):
            LOG.error(_LE('UCS Manager network driver failed to create '
    ...
    def _update_service_profile(self, host_id, service_profile, vlan_id):
    ...
    def update_serviceprofile(self, host_id, vlan_id):
        ...
        """
        service_profile = self.ucsm_conf.get_service_profile_for_host(host_id)
        if service_profile:
        ...
        if not self._update_service_profile(host_id, service_profile, vlan_id):
    ...
    def _delete_vlan_profile(self, host_id, vlan_id):
    ...
    def _delete_port_profile(self, host_id, port_profile):
        ...
        with self.ucsm_connect_disconnect(host_id) as handle:
            try:
                ...
                if not p_profile:
                    LOG.info(_LI('UCS Manager network driver did not find '
    ...
    def _remove_vlan_from_all_service_profiles(self, host_id, vlan_id):
    ...
    def delete_all_config_for_vlan(self, vlan_id, port_profile):
        ...
        host_list = self.ucsm_conf.get_all_configured_hosts()
        for host_id in host_list:
            self._delete_port_profile(host_id, port_profile)
            self._remove_vlan_from_all_service_profiles(host_id, vlan_id)
            self._delete_vlan_profile(host_id, vlan_id)
