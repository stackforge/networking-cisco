...
class CiscoUcsmDriver(object):
    ...
    def _check_vnic_type_and_vendor_info(self, vnic_type, profile):
    ...
    def _update_service_profile(self, handle, service_profile, vlan_id):
        service_profile_path = SERVICE_PROFILE_PATH_PREFIX + service_profile
        eth0 = service_profile_path + ETH0
        eth1 = service_profile_path + ETH1
        ...
        if not obj:
            LOG.debug("UCS Manager network driver could find Service "
    ...
    def _delete_port_profile(self, handle, vlan_id, port_profile):
        port_profile_dest = (const.PORT_PROFILESETDN + const.VNIC_PATH_PREFIX +
                             profile_name)
    ...
    def delete_all_config_for_vlan(self, vlan_id, port_profile):
        ...
        try:
            handle.StartTransaction()
            self._delete_port_profile(handle, vlan_id, port_profile)
            self._remove_vlan_from_all_service_profiles(handle, vlan_id)
            self._delete_vlan_profile(self, handle, vlan_id)
