...
class CiscoUcsmDriver(object):
    ...
    def ucs_manager_connect(self, ucsm_ip):
        ...
        if not credentials:
            ...
            LOG.debug('UCSM network driver Username : %s, Password; %s',
    ...
    def _update_service_profile_template(self, vlan_id, host_id, ucsm_ip):
        ...
        sp_template_path = (
            ...
            str('/ls-') + sp_template)
        LOG.debug('SP Template with path : %s', sp_template_path)
        ...
        with self.ucsm_connect_disconnect(ucsm_ip) as handle:
            try:
                ...
                for eth_port_path in eth_port_paths:
                    LOG.debug('Eth Port path: %s', eth_port_path)
                    eth = handle.GetManagedObject(
                        obj, self.ucsmsdk.VnicEther.ClassId(),
                        {self.ucsmsdk.VnicEther.DN: eth_port_path}, True)
    ...
    def update_vnic_template(self, host_id, vlan_id, physnet):
        ...
        vnic_template_path, vnic_templates = (
                    self.ucsm_conf.get_vnic_templates_for_physnet(ucsm_ip,
                    physnet))
        ...
        with self.ucsm_connect_disconnect(ucsm_ip) as handle:
            try:
                ...
                for vnic_template in vnic_template_list:
                    ...
                    if not eth_if:
                        LOG.error(_LE('UCS Manager network driver could '
                            'not add VLAN %(vlan_name)s to VNIC template '
                            '%(vnic_template_path)s at'),
                            {'vlan_name': vlan_name,
                            'vnic_template_path': vnic_template_path})
                handle.CompleteTransaction()
                return True
            except Exception as e:
    ...
    def _remove_vlan_from_vnic_templates(self, vlan_id, ucsm_ip):
        ...
        with self.ucsm_connect_disconnect(ucsm_ip) as handle:
            try:
                ...
                for temp_info in vnic_template_info:
                    ...
                    LOG.debug('SD: vnic_template_list: %s', vnic_template_list)
                    LOG.debug('SD: vnic_template_partial_path: %s',
                        vnic_template_path)
                    for vnic_template in vnic_template_list:
                        vnic_template_path += (const.VNIC_TEMPLATE_PREFIX +
                        str(vnic_template))
                        LOG.debug('SD: vnic_template_full_path: %s',
                        vnic_template_path)
                        mo = handle.GetManagedObject(
                            None,
                            self.ucsmsdk.VnicLanConnTempl.ClassId(),
                        ...
                        if not mo:
                            LOG.error(_LE('UCS Manager network driver could '
                                'not find VNIC template %s at'),
                                vnic_template_path)
                        vlan_dn = (vnic_template_path +
                            const.VLAN_PATH_PREFIX + str(vlan_name))
                        LOG.debug('SD: VNIC Template VLAN path; %s', vlan_dn)
                        ...
                        if not eth_if:
                            LOG.error(_LE('UCS Manager network driver could '
                                'not delete VLAN %(vlan_name)s from VNIC '
                                'template %(vnic_template_path)s at'),
                                {'vlan_name': vlan_name,
                                'vnic_template_path': vnic_template_path})
                        if eth_if:
    ...
    def delete_all_config_for_vlan(self, vlan_id, port_profile):
        ...
        for ucsm_ip in ucsm_ips:
            ...
            if self.ucsm_conf.is_vnic_template_configured():
                self._remove_vlan_from_vnic_templates(vlan_id, ucsm_ip)
            else:
