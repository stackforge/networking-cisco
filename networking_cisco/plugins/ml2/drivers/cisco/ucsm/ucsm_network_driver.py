...
class CiscoUcsmDriver(object):
    ...
    def _ssl_cert_check(self):
        if not config.get_ucsm_https_verify():
            try:
                _create_unverified_https_context = ssl._create_unverified_context
            except AttributeError:
                # Python versions that don't verify HTTPS certificates by default
                pass
            else:
                # Handle target environment that doesn't support HTTPS verification
                LOG.warning('SSL certificate verification has been disabled for the '
                            'UCSM plugin. The connection to UCS Manager(s) is now '
                            'insecure.')
