...
class CiscoUcsmDriver(object):
    ...
    def _delayed_delete_port_profile(self):
        pp_delete_dict = {}
        LOG.debug('SD: Inside delayed delete port profile.')
        Timer(const.DEFAULT_PP_DELETE_TIME,
            self._delayed_delete_port_profile, ()).start()
        all_pps = self.ucsm_db.get_all_port_profiles_to_delete()
        for pp in all_pps:
            key = pp.device_id
            if pp.device_id not in pp_delete_dict.keys():
                pp_delete_dict[key] = []

            pp_delete_dict[key].append(pp.profile_id)
            LOG.debug('SD: %s :Adding port profile. %s', pp.device_id, pp.profile_id)

        # Connect to each UCSM IP and try to delete Port profiles
        for ucsm_ip in pp_delete_dict.keys():
            LOG.debug('SD: Connecting to UCSM_IP : %s', ucsm_ip)
            with self.ucsm_connect_disconnect(ucsm_ip) as handle:
    ...
    def _delete_port_profile(self, handle, port_profile, ucsm_ip):
        ...
        try:
            ...
            # Remove this Port Profile from the delete DB table if it was
            # addded there due to a previous delete.
            LOG.debug('SD: Removing PP %s from delete table after successful delete.', port_profile)
            ...
            # Raise a Neutron exception. Include a description of
            # the original  exception.
            LOG.debug('SD: Received Port Profile delete exception for %s', port_profile)
            LOG.debug('SD: Received Port Profile delete exception %s', e)
            self.ucsm_db.add_port_profile_to_delete_table(port_profile, ucsm_ip)
