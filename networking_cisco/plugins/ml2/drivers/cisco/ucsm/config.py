...
def parse_virtio_eth_ports():
    eth_port_list = []
    LOG.debug("UCS Mech Driver: Parsing virtio eth ports.")
    if cfg.CONF.ml2_cisco_ucsm.virtio_eth_ports:
        for eth_port in cfg.CONF.ml2_cisco_ucsm.virtio_eth_ports:
            eth_port_list.append(const.ETH_PREFIX + str(eth_port))
            LOG.debug("UCS Mech Driver: Virtio eth ports: %s",
                      str(eth_port_list))
