...
cfg.CONF.register_opt(ucsms, "ml2_cisco_ucsm")
cfg.CONF.register_opts(sriov_opts, "sriov_multivlan_trunk")

UCSM_CONF = cfg.CONF.ml2_cisco_ucsm


def parse_pci_vendor_config():
    vendor_list = []
    vendor_config_list = UCSM_CONF.supported_pci_devs
...
class UcsmConfig(object):
    ...
    def ucsm_host_dict(self):
        host_dict = {}
        if UCSM_CONF.ucsm_ip:
            for host, sp in (UCSM_CONF.ucsm_host_list or {}).items():
                host_dict[host] = UCSM_CONF.ucsm_ip
        elif UCSM_CONF.ucsms:
            for ip, ucsm in UCSM_CONF.ucsms.items():
    ...
    def ucsm_sp_dict(self):
        ...
        if UCSM_CONF.ucsm_ip:
            for host, sp in (UCSM_CONF.ucsm_host_list or {}).items():
                if '/' not in sp:
                    sp_dict[(UCSM_CONF.ucsm_ip, host)] = (
                        const.SERVICE_PROFILE_PATH_PREFIX + sp.strip())
                else:
                    sp_dict[(UCSM_CONF.ucsm_ip, host)] = sp.strip()
        elif UCSM_CONF.ucsms:
            for ip, ucsm in UCSM_CONF.ucsms.items():
                for host, sp in (ucsm.ucsm_host_list or {}).items():
                    if '/' not in sp:
    ...
    def get_credentials_for_ucsm_ip(self, ucsm_ip):
        if ucsm_ip == UCSM_CONF.ucsm_ip:
            username = UCSM_CONF.ucsm_username
            password = UCSM_CONF.ucsm_password
        elif ucsm_ip in UCSM_CONF.ucsms:
            username = UCSM_CONF.ucsms[ucsm_ip].ucsm_username
            password = UCSM_CONF.ucsms[ucsm_ip].ucsm_password
        if username and password:
            return (username, password)

    def get_all_ucsm_ips(self):
        if UCSM_CONF.ucsm_ip:
            return [UCSM_CONF.ucsm_ip]
        elif UCSM_CONF.ucsms:
            return list(UCSM_CONF.ucsms)

    def get_ucsm_eth_port_list(self, ucsm_ip):
        conf = UCSM_CONF
        if ucsm_ip == UCSM_CONF.ucsm_ip:
            return map(lambda x: const.ETH_PREFIX + x,
    ...
    def add_sp_template_config_for_host(self, host, ucsm_ip,
        ...
        for ucsm, sps in ucsm_template_map.items():
            ...
            UCSM_CONF.ucsms[ucsm].sp_template_list = " ".join(entries)
    ...
    def _vnic_template_data_for_ucsm_ip(self, ucsm_ip):
        if ucsm_ip not in UCSM_CONF.ucsms:
            return []
        template_list = UCSM_CONF.ucsms[ucsm_ip].vnic_template_list
        mappings = []
    ...
    def get_sriov_qos_policy(self, ucsm_ip):
        if ucsm_ip in UCSM_CONF.ucsms:
            # NOTE(sambetts) Try to get UCSM specific SRIOV policy first else
            # return global policy
            return (UCSM_CONF.ucsms[ucsm_ip].sriov_qos_policy or
                    UCSM_CONF.sriov_qos_policy)
        else:
            return UCSM_CONF.sriov_qos_policy
