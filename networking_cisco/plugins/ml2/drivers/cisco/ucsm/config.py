...
ml2_cisco_ucsm_opts = [
    ...
    cfg.ListOpt('supported_pci_devs',
                ...
                help=_('SR-IOV and VM-FEX vendors to be handled by the driver '
                       'xxxx:yyyy represents vendor_id:product_id of the PCI '
                       'networking devices that the driver needs to handle. '
                       'It is implicit that the SR-IOV capable devices '
                       'specified here should be supported on the UCS '
                       'platform.')),
    cfg.BoolOpt('ucsm_https_verify',
                ...
                help=_('The UCSM driver will always perform SSL certificate '
                       ...
                       'global configuration to False. This configuration is '
                       'optional and will default to True when not specified. '
...
ml2_cisco_ucsm_common = [
    ...
    cfg.ListOpt('ucsm_virtio_eth_ports',
                ...
                help=_('Ethernet port names to be used for virtio ports. '
                       ...
                       'port configuration. This is an optional config and '
                       'defaults to eth0 and eth1. The names should match the '
                       'names on the UCS Manager.')),
    cfg.DictOpt('ucsm_host_list',
                help=_('Hostname to Service profile mapping for UCS Manager '
                       'controlled compute hosts. This Service profile '
    ...
    cfg.StrOpt('sriov_qos_policy',
               help=_('Pre-defined QoS policy name. This config allows the '
                      'cloud admin to pre-create a QoS policy on the UCSM. If '
                      'this config is present, the UCSM driver will associate '
                      'this QoS policy with every Port profile it creates for '
                      'SR-IOV ports.')),
    cfg.StrOpt('sp_template_list',
               help=_('Service Profile Template config for this UCSM. The '
                      ...
                      'controlled by this template. This is an optional '
    ...
    cfg.StrOpt('vnic_template_list',
               help=_('VNIC Profile Template config per UCSM. Allows the '
                      ...
                      'this physical network. This is an optional config with '
                      'no defaults.')),
]
...
sriov_opts = [
    base.RemainderOpt('network_name',
                      dest="network_vlans",
                      help=_('SR-IOV Multi-VLAN trunk config section. This '
                             'config section was specifically added to '
                             'accomodate the scenario where an application '
                             'using an SR-IOV port to communicate, would like '
                             'to send traffic on multiple application '
                             'specific VLANs not known to OpenStack. This '
                             'config section is applicable across all UCSMs '
                             'specified as part of the OpenStack cloud. The '
                             'names of the neutron networks on which the '
                             'SR-IOV ports are going to be created have to be '
                             'known ahead of time and should be associated '
                             'with a list or range of application VLANs using '
                             'the following format: '
                             '<neutron network name>=<comma separated list of '
                             'VLAN-ids or VLAN-id ranges> '
                             'For example: test_network1=5,7-9')),
]
