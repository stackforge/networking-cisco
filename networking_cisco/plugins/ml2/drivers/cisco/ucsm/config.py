...
class UcsmConfig(object):
        ...
        for sp_template_temp in sp_template_config:
            ...
            for sp_template in sp_template_list:
                ...
                for host in host_list:
                    ...
                    LOG.debug('SP Template key: %s, value: %s',
                        host, value)
    ...
    def get_sp_template_path_for_host(self, host):
        ...
        LOG.debug('SP Template Path is %s for host %s', value[1], host)
        return value[1]

    def get_sp_template_for_host(self, host):
        value = self.sp_template_dict.get(host)
        LOG.debug('Host %s is attached to SP Template %s', host, value[2])
        return value[2]

    def get_ucsm_ip_for_sp_template_host(self, host):
        value = self.sp_template_dict.get(host)
        LOG.debug('Host %s belongs to UCSM %s', host, value[0])
        return value[0]
    ...
    def _parse_vnic_template_list(self, ucsm_ip, vnic_template_config):
        ...
        for vnic_template_temp in vnic_template_config:
            ...
            for mapping in vnic_template_mapping:
                ...
                vnic_template_path, sep, vnic_template_names = (
                    vnic_templates.partition(':'))
                if not vnic_template_path:
                    vnic_template_path = const.VNIC_TEMPLATE_PARENT_DN
                if not vnic_template_names:
                    raise cfg.Error(_("UCS Mech Driver: Invalid VNIC Template"
                                      "names for physnet: %s") % physnet)

                key = (ucsm_ip, physnet)
                value = (vnic_template_path, vnic_template_names)
    ...
    def get_vnic_templates_for_ucsm_ip(self, ucsm_ip):
        vnic_template_info_list = []
        keys = self.vnic_template_dict.keys()
        for key in keys:
            LOG.debug('SD: For key : %s,', key)
            value = self.vnic_template_dict.get(key)
            LOG.debug('SD: Appending VNIC Template %s to the list.', value[1])
            vnic_template_info_list.append(self.vnic_template_dict.get(key))
