...
ml2_cisco_ucsm_opts = [
    cfg.StrOpt('ucsm_ip',
               help=_('Cisco UCS Manager IP address. This is required field '
                      'to communicate with a Cisco UCS Manager')),
    cfg.StrOpt('ucsm_username',
               help=_('Username for UCS Manager. This is required field '
                      'to communicate with a Cisco UCS Manager')),
    cfg.StrOpt('ucsm_password',
               secret=True,  # do not expose value in the logs
               help=_('Password for UCS Manager. This is required field '
                      'to communicate with a Cisco UCS Manager')),
    cfg.ListOpt('supported_pci_devs',
                ...
                help=_('List of comma separated vendor_id:product_id of '
                       'devices supported by this MD. This MD supports '
                       'VM-FEX and SR-IOV devices')),
...
def parse_pci_vendor_config():
    ...
    for vendor in vendor_config_list:
        ...
        if len(vendor_product) != 2:
            raise ValueError(_("UCS Mech Driver: Invalid PCI device "
                               "config: %s"), vendor)
...
def parse_ucsm_host_config():
    ...
    for host in host_config_list:
        host_sp = host.split(':')
        if len(host_sp) != 2:
            raise ValueError(_("UCS Mech Driver: Invalid Host Service "
