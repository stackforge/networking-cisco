...
class CiscoNexusDriver(object):
    ...
    def capture_and_print_timeshot(self, start_time, which,
        ...
        if which in self.time_stats:
            ...
            self.time_stats[which] = {
                ...
                "min": curr_timeout,
                "max": curr_timeout}
        LOG.debug("CB_TIME_STATS %s: %s_timeout %f count %d "
    ...
    def initialize_all_switch_interfaces(self, interfaces):
        ...
        for nexus_host, intf_type, nexus_port in interfaces:
            if self.get_interface_switch_trunk_allowed(
                nexus_host, intf_type, nexus_port) is False:
    ...
    def get_create_vlan(self, nexus_host, vlanid, vlanname, vni):
    ...
    def create_vlan_segment(self, nexus_host,
                            vlanid, vlanname, vni):
        ...
        # VLANS are now pre-activated at initialization.  Only
        # VXLAN configuration of vn-segment is needed.
        if vni == 0:
            return

        starttime = time.time()
        confstr = self.get_create_vlan(nexus_host, vlanid, vlanname, vni)
    ...
    def delete_vlan_segment(self, nexus_host, vlanid, vni):
        """Delete a VLAN segment on Nexus Switch given the VLAN ID."""

        # VLANS are now pre-activated at initialization.  Only
        # VXLAN configuration of vn-segment needs removal.
        if vni == 0:
            return

        confstr = snipp.CMD_NO_VLAN_SEGMENT_SNIPPET % vlanid
    ...
    def create_and_trunk_vlan(self, nexus_host, vlan_id, vlan_name,
        ...
        starttime = time.time()
        self.create_vlan_segment(nexus_host, vlan_id, vlan_name, vni)
        LOG.debug("NexusDriver created VLAN: %s", vlan_id)
        if nexus_port:
            confstr = self.get_enable_vlan_on_trunk_int(nexus_host, vlan_id,
