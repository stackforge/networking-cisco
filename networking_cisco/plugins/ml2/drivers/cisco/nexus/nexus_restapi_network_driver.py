...
class CiscoNexusRestapiDriver(basedrvr.CiscoNexusBaseDriver):
    ...
    def __init__(self, nexus_dict):
        super(CiscoNexusRestapiDriver, self).__init__(nexus_dict)
        credentials = self._build_credentials(self.nexus_dict)
    ...
    def _get_user_port_channel_config(self, switch_ip, vpc_nbr):
        ...
        ucmds = self.nexus_dict.get((switch_ip, const.IF_PC))
        if ucmds:
