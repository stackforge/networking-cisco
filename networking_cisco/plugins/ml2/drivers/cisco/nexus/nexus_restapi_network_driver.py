...
class CiscoNexusRestapiDriver(basedrvr.CiscoNexusBaseDriver):
    ...
    def __init__(self):
        ...
        self.nxapi_client = self._import_nxapi_client(credentials)
    ...
    def _import_nxapi_client(self, credentials):
        """Import the local NXAPI CLI client module.
        This method was created to handle NXAPI CLI module.
        """
    ...
    def _get_user_port_channel_config(self, switch_ip, vpc_nbr):
        ...
        :param nexus_switches: switch config
        :returns ucmds:      port channel commands or None
        """
        nexus_dict = conf.ML2MechCiscoConfig.nexus_dict
        if (switch_ip, const.IF_PC) not in nexus_dict:
            return None
    ...
    def _send_cli_conf_string(self, nexus_host, cli_str):
        """Sends CLI Config command to Nexus switch."""
