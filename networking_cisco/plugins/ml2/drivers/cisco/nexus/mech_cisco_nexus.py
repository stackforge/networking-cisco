...
class CiscoNexusMechanismDriver(api.MechanismDriver):
    ...
    # 2) The port reserved binding is used with baremetal
    ...
    def _supported_baremetal_transaction(self, context):
        ...
        for i in range(len(link_info)):
            ...
            if 'port_id' not in link_info[i]:
                return False

            if 'switch_info' not in link_info[i]:
                return False

            switch_info = link_info[i]['switch_info']
            if 'switch_ip' not in switch_info:
                return False

            # TODO(caboucha) look for
            #if 'system_desc' not in switch_info:
            #    return False
            # 'Nexus Operating System' in system_desc
            #    return False
        ...
        for segment in context.segments_to_bind:
            ...
            # not valid vlan_id
            if not segment[api.SEGMENTATION_ID]:
                continue
    ...
    def _get_baremetal_switches(self, port):
        ...
        all_switches = set()
        active_switches = set()
        link_info = port[portbindings.PROFILE]['local_link_information']
        for i in range(len(link_info)):
            switch_info = link_info[i]['switch_info']
    ...
    # following cases.
    # 1) If RESERVED port has zero and switch returns non-zero, then
    ...
    # 2) If RESERVED port has non-zero and switch returns non-zero
    ...
    #      RESERVED port.
    # 3) If RESERVED port has non-zero and switch returns zero, then:
    ...
    def bind_port(self, context):
        #CB_TBD need to set 'id' in FakeContext
