...
class CiscoNexusCfgMonitor(object):
    ...
    def replay_config(self, switch_ip):
        ...
        self._mdriver.gather_configured_vlan_range(switch_ip)

        try:
            # Before replaying all config, the following determines
            # if the switch interfaces are in place.
            # If so, make sure they have a basic trunk
...
class CiscoNexusMechanismDriver(api.MechanismDriver):
    ...
    def is_reserved_binding(self, binding):
        if binding.instance_id == const.RESERVED_NEXUS_SWITCH_DEVICE_ID_R1:
            return True
        else:
            return False
    ...
    def is_switch_active(self, switch_ip):
        ...
        if switch_state in active_states:
            return True
        else:
            return False
    ...
    def save_switch_vlan_range(self, switch_ip, vlan_range):
        self._switch_state[switch_ip, '_vlan_range'] = vlan_range

    def get_switch_vlan_range(self, switch_ip):
        if (switch_ip, '_vlan_range') in self._switch_state:
            return self._switch_state[switch_ip, '_vlan_range']
        else:
            return []

    def gather_configured_vlan_range(self, switch_ip):
        """Gather the configured vlan range from ml2_conf.ini file.

        Input: physnet1:190-192,physnet2:194-196
        Result : A list of tupled (from, to) variables.
                 [(190, 192), (194,196)]
        Store result in switch '_vlan_range' data.
        """
        nexus_range = []
        for segment in cfg.CONF.ml2_type_vlan.network_vlan_ranges:
            # if count is not 3 or range values are not digit,
            # I don't know what we have. Skip it.
            cfg_range = segment.split(':')
            if (len(cfg_range) != 3 or not cfg_range[1].isdigit() or
                not cfg_range[2].isdigit()):
                continue
            vlan_fr = int(cfg_range[1])
            vlan_to = int(cfg_range[2])
            if vlan_fr > vlan_to:
                continue
            nexus_range.append((vlan_fr, vlan_to))
        self.save_switch_vlan_range(switch_ip, nexus_range)

    def pop_vlan_range(self, switch_ip, size):
        ...
        Sample Use Cases:
        1) If stored range is 1000, 4000 and size is 100,
        then 100-199 is returned and 1200-4000 pushed
        back into storage.
        2) if stored range is 2-3,7-10 and size is 4,
        then 2-3,7-8 is returned and 9-10 pushed
        back into storage for next time.
        """
        nexus_range = self.get_switch_vlan_range(switch_ip)
        sized_range = ''
        # if nexus_range not empty and haven't met requested size
        while size > 0 and nexus_range:
            vlan_fr, vlan_to = nexus_range.pop(0)
            diff = vlan_to - vlan_fr + 1
            new_fr = vlan_fr
            if diff > size:
                vlan_fr += size
                new_to = vlan_fr - 1
                # push remainder to beginning of list of tuples
                nexus_range.insert(0, (vlan_fr, vlan_to))
            else:
                new_to = vlan_to
            ...
            if new_fr == new_to:
                if sized_range == '':
                    sized_range += str(new_fr)
                else:
                    sized_range += ',' + str(new_fr)
            else:
                if sized_range == '':
                    sized_range += str(new_fr) + '-' + str(new_to)
                else:
                    sized_range += ',' + str(new_fr) + '-' + str(new_to)
    ...
    def _configure_port_binding(self, is_provider_vlan, duplicate_type,
                                switch_ip, vlan_id,
                                intf_type, nexus_port, vni, confstr):
        ...
        if auto_create and auto_trunk:
            ...
            confstr += self.driver.create_and_trunk_vlan(
            ...
            confstr += self.driver.get_enable_vlan_on_trunk_int(
                           switch_ip, vlan_id,
    ...
    def _restore_vlan_entries(self, switch_ip, vlans):
        """Restore vlan entries on a Nexus switch."""

        count = 1
        conf_str = ''
        vnsegment_sent = 0
        # At this time, this will only configure vni information when needed
        for vlan_id, vni, vlan_name in vlans:
            # Add it to the batch
        ...
        ## CB_TBD  Do we want to compare against current config?
        ## The following does this but uses 'show vlan' when
        ## 'show run vlan' would be better since the latter
        ## contains the vn-segment.
        #    matched = 0
        #    unmatched = 0
        #    other = 0
        #    vlan_list = self.driver.get_current_vlans(switch_ip)
        #    for vlan_id, vni, vlan_name in vlans:
        #        if vlan_id in vlan_list:
        #            name, state, shut = vlan_list[vlan_id]
        #            # Skip if already configured on host
        #            if (vlan_name == name and
        #                state == 'active' and
        #                shut == 'noshutdown'):
        #                matched += 1
        #                continue
        #            elif (vlan_name == name):
        #                # Send this alone since it will cause duplicate error
        #                # and prevent remainder of batch to fail
        #                self.driver.create_vlan_segment(
        #                    switch_ip, vlan_id, vlan_name, vni)
        #                other += 1
        #                continue
        #        unmatched += 1
        ...
        #        self.driver.send_edit_string(switch_ip, conf_str)
        #        conf_str = ''
        #    LOG.debug("Switch %s VLAN replay summary: matched %d, "
        #              "unmatched %d, other %d",
        #              switch_ip, matched, unmatched, other)
        # Set the state to all possible vlans to active/noshut
        # User may have to set Nexus switch to
        # 'spanning-tree mode mst' depending on range
        #nexus_range = ''
        #for segment in cfg.CONF.ml2_type_vlan.network_vlan_ranges:
        #    # if count is not 3 or range values are not digit,
        #    # I don't know what we have. Skip it.
    ...
    def _configure_host_entries(self, vlan_id, device_id, host_id, vni,
        ...
        for switch_ip, intf_type, nexus_port in host_connections:
            ...
            try:
                confstr = self._configure_port_binding(
            ...
            self.driver.capture_and_print_timeshot(
                ...
                switch=switch_ip)
        if confstr:
            self.driver.send_edit_string(switch_ip, confstr)
            confstr = ''
        self.driver.capture_and_print_timeshot(
            starttime, "config",
            switch=switch_ip)
    ...
    def configure_switch_entries(self, switch_ip, port_bindings):
        ...
        for port in port_bindings:
            if self.is_reserved_binding(port):
                continue
            vlan_name, auto_create, auto_trunk = self._gather_config_parms(
        ...
        self._restore_vlan_entries(switch_ip, vlans)
    ...
    def _delete_switch_entry(self, vlan_id, device_id, host_id, vni,
        ...
        for switch_ip, intf_type, nexus_port in host_connections:
            ...
            # if there are no remaining db entries using this vlan on this
            # nexus switch then remove the vni from the vlan.
            if auto_create:
                try:
                    ...
                    if switch_ip not in vlan_already_removed:
                        self.driver.delete_vlan_segment(
        ...
        if self._is_supported_deviceowner(port):
            ...
            for switch_ip, intf_type, nexus_port in host_connections:
                if not self.is_switch_active(switch_ip):
                    raise excep.NexusConfigFailed(
                        nexus_host=switch_ip, config="None",
                        exc="Create Failed: Nexus Switch is down "
