...
class CiscoNexusMechanismDriver(api.MechanismDriver):
    ...
    def save_switch_vxlan_range(self, switch_ip, vlan_range):
        self._switch_state[switch_ip, '_vxlan_range'] = vlan_range

    def get_switch_vxlan_range(self, switch_ip):
    ...
    def pop_vlan_range(self, switch_ip, size):
        ...
        """
        vlan_range = self.get_switch_vlan_range(switch_ip)
        sized_range = ''
        fr = 0
        to = 0
        ...
        if fr != 0:
            if fr == to:
                ...
                sized_range += str(fr) + '-'
                sized_range += str(to)
            self.save_switch_vlan_range(switch_ip, vlan_range)
    ...
    def configure_next_batch_of_vlans(self, switch_ip):
        ...
        next_range = self.pop_vlan_range(
        ...
        vxlan_range = self.get_switch_vxlan_range(switch_ip)
        self._restore_vxlan_entries(switch_ip, vxlan_range)

        # if no more vlans to restore, we're done. go active.
        if (not self.get_switch_vlan_range(switch_ip) and
            not self.get_switch_vxlan_range(switch_ip)):
    ...
    def configure_switch_entries(self, switch_ip, port_bindings):
        ...
        if vlans:
            ...
            if vni == 0:
                self.save_switch_vlan_range(switch_ip, vlans)
            else:
                self.save_switch_vxlan_range(switch_ip, vlans)
