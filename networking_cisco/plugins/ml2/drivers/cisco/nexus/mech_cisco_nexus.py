...
class CiscoNexusMechanismDriver(api.MechanismDriver):
    ...
    def _get_configured_switch_vpc_list(self, switch_ip):
        # Collect existing list of vpcs for a given switch.

        try:
            vpc_list = nxos_db.get_all_switch_vpc_allocs(switch_ip)
        except excep.NexusVPCAllocNotFound:
            vpc_list = []
        return vpc_list

    def _valid_vpc_alloc_config(self, switch_ip):
        ...
        vpc_range = [test for test in vpc_range if test != '']
        if not vpc_range:
            return [], False
        for vpcid in vpc_range:
            ...
            if ((len(minmax) == 1 and not minmax[0].isdigit()) or
                (len(minmax) == 2 and (not minmax[0].isdigit() or
                not minmax[1].isdigit()))):
                LOG.error(_LE("Non-numeric value %(bad)s configured "
            ...
            if (start >= 1 and start <= 4096 and
                end >= 1 and end <= 4096 and
                end >= start):
                pass
            else:
                LOG.error(_LE("Invalid Port-channel value %(bad)s received "
                          "in vpc-pool config %(all)s for switch "
                          "%(switchip)s. Ignoring entire config."),
                          {'bad': vpcid, 'all': value,
                          'switchip': switch_ip})
        ...
        # put unique vpcs in order
        new_list = list(new_list)
        new_list.sort()
        return new_list, False

    def _compare_vncpool_lists(self, old_list, new_list):
    ...
    def _get_vpcpool_changes_needed(self, switch_ip):
        ...
        old_list = self._get_configured_switch_vpc_list(
            switch_ip)

        # Get list of configured vpcs desired
        new_list, error = self._valid_vpc_alloc_config(switch_ip)
        ...
        return self._compare_vncpool_lists(old_list, new_list)
