...
class CiscoNexusMechanismDriver(api.MechanismDriver):
    ...
    def initialize(self):
        LOG.debug("nexus_switches found = %s", CONF.ml2_cisco.nexus_switches)
    ...
    def get_nve_loopback(self, switch_ip):
        return CONF.ml2_cisco.nexus_switches[switch_ip].get(const.NVE_SRC_INTF, '0')
        ...
        for segment in context.segments_to_bind:
            if self._is_segment_nexus_vxlan(segment):
                ...
                for switch_ip, _, _, _, _ in host_connections:
                    physnet = CONF.ml2_cisco.nexus_switches[switch_ip].get(const.PHYSNET)
                    if physnet:
                        break
                else:
