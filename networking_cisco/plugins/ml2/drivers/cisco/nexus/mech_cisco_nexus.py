    ...
    def __init__(self, driver, mdriver):
        ...
        for switch_ip in switch_connections:
            ...
            if not self._mdriver.is_replay_enabled():
                ...
                self._mdriver.configure_next_batch_of_vlans(
                    switch_ip, all=True)
                try:
...
class CiscoNexusMechanismDriver(api.MechanismDriver):
    ...
    def is_replay_enabled(self):
        if conf.cfg.CONF.ml2_cisco.switch_heartbeat_time > 0:
            return True
        else:
            return False
    ...
    def configure_next_batch_of_vlans(self, switch_ip, all=False):
        """Get next batch of vlans and send them to Nexus."""

        size = (const.CREATE_FULL_VLAN_BATCH if all is True else
               const.CREATE_VLAN_BATCH)
