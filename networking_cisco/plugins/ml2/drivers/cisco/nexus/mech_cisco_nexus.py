            ...
            if str(attr) == const.VPCPOOL:
                try:
                    countvpc, minvpc, maxvpc = (
                        nxos_db.get_switch_vpc_allocs_count_min_max(
                            switch_ip)
                    )
                ...
                elif (countvpc != count or str(minvpc) != fromvpc or
                     str(maxvpc) != tovpc):
                    LOG.info(_LI("Cannot resize switch vpc_pool config "
                                 "from %(today)d to %(chg)d entries. "
                                 "Ignoring config change for "
                                 "switch %(ip)s."), {'today': countvpc,
                                 'chg': count, 'ip': switch_ip})
