...
nexus_sub_opts = [
    cfg.BoolOpt('https_verify', default=False, deprecated_for_removal=True,
        deprecated_reason='Default changing from False to True',
        help=_('This is marked for deprecation since the default '
               'configuration is False to be backward compatible but '
        ...
        for switch_ip, switch in cfg.CONF.ml2_cisco.nexus_switches.items():
            for opt_name, value in switch.items():
                if opt_name == 'compute_hosts':
                    ...
                    #need to save False for this option
                    self.nexus_dict[(switch_ip, opt_name)] = value
