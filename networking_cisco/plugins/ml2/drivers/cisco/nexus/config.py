...
nexus_sub_opts = [
    cfg.StrOpt('intfcfg.portchannel',
        help=_('This variable is dependent on "vpc_pool" being configured.'
               'It is a string of Nexus port-channel config cli for use when '
               'baremetal port-channels are created by the Nexus driver. Any '
               'number of Nexus port-channel commands separated by ";" can be '
    ...
    cfg.StrOpt('nve_src_intf',
        help=_('Only valid if VXLAN overlay is configured and '
               ...
               'non-baremetal only.')),
    cfg.StrOpt('password',
        help=_('The password is a required variable of the Nexus Switch '
               'administrator needed to allow configuration access to the '
               'Nexus switch. This variable is required for baremetal and '
               'non-baremetal deployments.')),
    cfg.StrOpt('physnet',
    ...
    cfg.StrOpt('username',
        help=_('The user name is a required variable of the Nexus Switch '
               'administrator needed to allow configuration access to the '
               'Nexus switch. This variable is required for baremetal and '
               'non-baremetal deployments.')),
    cfg.StrOpt('vpc_pool',
    ...
    base.RemainderOpt('compute_hosts',
        help=_('Unlike other variables in this section, compute_hosts is '
               'a wild card.  If you define a variable which is not any of '
               'the other switch variables, it is assumed to be the name '
               'of a compute host. So you would never configure anything like '
               'compute_hosts=x. If you did, the assumption would be you have '
               'a compute host with the name "compute_hosts".  There can be '
               'multiple compute hosts for each switch configured. For each '
               'host connected to a port on the switch, specify the hostname '
               'and the Nexus switch physical port (interface) it is '
               ...
               'have multiple compute hosts configured on the same switch, '
               'they must each be configured on a different line in the '
               'config file.  There is no special character to separate them '
...
ml2_cisco_opts = [
    cfg.BoolOpt('host_key_checks', default=False, deprecated_for_removal=True,
        help=_('Set to True to enable strict hostkey checks when connecting '
               'to Nexus switches; otherwise, no hostkey checks are performed '
               'by default. checks). This will be deprecated along with '
    ...
    cfg.StrOpt('nexus_driver', default='restapi', deprecated_for_removal=True,
        help=_('The Nexus Mechanism Driver has two driver methods to '
               ...
               'driver can be used by setting the "nexus_driver" variable to '
               '"ncclient".  This is short term because the "ncclient" driver '
               'will be removed in Cisco 7.0.0 release of the '
               'networking-cisco repository.')),
    ...
    cfg.IntOpt('switch_heartbeat_time', default=30,
        help=_('Time interval to check the state of all known Nexus '
               'device(s). This variable defaults to 30 seconds which checks '
               'each Nexus device every 30 seconds.  To disable configuration '
               'replay, set this variable to 0 seconds.')),
