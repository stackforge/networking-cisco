...
class NetworkProfile_db_mixin(network_profile_module.NetworkProfilePluginBase,
    ...
    def _get_network_collection_for_tenant(self, db_session, model, tenant_id):
        # add bindings for default network profiles with this tenant if absent
        for net_prof_name in [
                n1kv_const.DEFAULT_VLAN_NETWORK_PROFILE_NAME,
                n1kv_const.DEFAULT_VXLAN_NETWORK_PROFILE_NAME]:
            net_prof = n1kv_db.get_network_profile_by_name(net_prof_name,
                                                           db_session)
            try:
                n1kv_db.get_profile_binding(db_session, tenant_id, net_prof.id)
            except n1kv_exc.ProfileTenantBindingNotFound:
                n1kv_db.add_profile_tenant_binding(
                    profile_type='network',
                    profile_id=net_prof.id,
                    tenant_id=tenant_id,
    ...
    def _create_profile_binding(self, db_session, tenant_id, profile_id):
        ...
        db_session = db_session or db.get_session()
        try:
            binding = n1kv_db.get_profile_binding(db_session, tenant_id,
                                                  profile_id)
        except n1kv_exc.ProfileTenantBindingNotFound:
            with db_session.begin(subtransactions=True):
