...
def get_profile_bindings(db_session, tenant_id, profile_type):
    bindings = (db_session.query(n1kv_models.ProfileBinding).filter_by(
        tenant_id=tenant_id, profile_type=profile_type).all())
    bindings = [dict([(k, v) for (k, v) in d.items() if k !=
                      'profile_type']) for d in bindings]
    return bindings
