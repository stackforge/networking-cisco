...
class PolicyProfilePlugin(PolicyProfile_db_mixin):
    ...
    def get_policy_profiles(self, context, filters=None, fields=None):
        ...
        :param filters: a dictionary with keys that are valid keys for a subnet
            object. Values in this dictiontary are an iterable containing values
            that will be used for an exact match comparison for that value. Each
            result returned by this function will have matched one of the values
