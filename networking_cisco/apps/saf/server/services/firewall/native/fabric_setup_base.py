...
class FabricApi(object):
    """
    Class for retrieving the FW attributes, available to external modules.
    """
    serv_obj_dict = {}
    ip_db_obj = {}
...
class FabricBase(dfa_dbm.DfaDBMixin, FabricApi):
    ...
    def get_gateway(self, subnet):
        ...
        address configured in the leaf. This value is subnet + 1.
    ...
    def get_next_ip(self, tenant_id, direc):
        ...
        return {'subnet': ip_next, 'start': self.get_start_ip(ip_next),
                ...
                'sec_gateway': self.get_sec_gateway(ip_next)}
    ...
    def fill_dcnm_subnet_info(self, tenant_id, subnet, start, end, gateway,
        ...
        if direc == 'in':
            ...
            name = fw_id[0:4] + fw_const.OUT_SERVICE_SUBNET + (
                fw_id[len(fw_id) - 4:])
        subnet_dict = {'enable_dhcp': False, 'tenant_id': tenant_id,
                       'allocation_pools': {}, 'name': name,
                       'cidr': subnet + '/24', 'gateway_ip': gateway,
                       'secondary_gw': sec_gateway, 'ip_version': 4}
        alloc_pool_dict = {}
    ...
    def _create_os_nwk(self, tenant_id, tenant_name, direc, is_fw_virt=False):
        ...
        # Step 1
        # Allocate IP with source not filled. This subnet information is not
        # yet stored in DB because there's no net ID, which is the primary key.
        # 'net_id' is obtained only after the openstack network is created.
        subnet_dict = self.alloc_retrieve_subnet_info(tenant_id, direc)

        # Step 2
        # Fill net parameters w/o vlan, seg allocated (becos we don't have
        # netid to store in DB)
        net_dict = self.fill_retrieve_net_info(tenant_id, direc)

        # Step 3
        # Create OS Network which gives NetID
        try:
            ...
            LOG.error(_LE("Create network for tenant %(tenant)s "
                      "network %(name)s direct %(dir)s failed exc %(exc)s "),
        ...
        # Step 4 (This fills the Seg/Vlan DB with net_id)
        ...
        # Step 5 (Updating IP DB with netid)
        # After creating OS network, update IP DB with netid as source,
