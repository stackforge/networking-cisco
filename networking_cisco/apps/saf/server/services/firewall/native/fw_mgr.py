...
class FwMgr(dev_mgr.DeviceMgr):
    ...
    def fw_exc_decorate(fw_func):
        @wraps(fw_func)
        def fw_handler_fn(inst, *args):
            try:
                fn_name = fw_func.func_name
                fw_func(inst, args)
            except Exception as exc:
    ...
    @fw_exc_decorate
    def _fw_policy_delete(self, args):
    ...
    @fw_exc_decorate
    def _fw_policy_create(self, args):
