...
class Asa5585(object):
    ...
    def build_acl_ip(self, ip_address):
        "Build the acl for IP address. "

        network_obj = netaddr.IPNetwork(ip_address)
        if ip_address == '0.0.0.0/0':
            acl = "any "
        else:
            acl = "%(ip)s %(mask)s " % {'ip': network_obj.network,
                                        'mask': network_obj.netmask}
        return acl, network_obj

    def build_acl_port(self, port_type, port, enabled=False):
    ...
    def build_acl(self, tenant_name, rule):
        ...
        ip_acl, src_network_obj = self.build_acl_ip(src_ip)
        acl += ip_acl

        acl += self.build_acl_port('src', rule.get('source_port'))

        dst_ip = self.get_ip_address(rule.get('destination_ip_address'))
        ip_acl, dst_network_obj = self.build_acl_ip(dst_ip)
        acl += ip_acl

        acl += self.build_acl_port('dst', rule.get('destination_port'),
