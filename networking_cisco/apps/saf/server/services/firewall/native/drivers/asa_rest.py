...
class Asa5585(object):
    ...
    def build_acl_ip(self, rule, type_ip):
        "Build the acl for IP address. "
        if type_ip == 'src':
            key = 'source_ip_address'
        else:
            key = 'destination_ip_address'
        if (key in rule and rule[key] is not None):
            ip_obj = netaddr.IPNetwork(rule[key])
        else:
            ip_obj = netaddr.IPNetwork('0.0.0.0/0')
        if (key not in rule or rule[key] is None):
            acl = "any "
        else:
            acl = "%(ip)s %(mask)s " % {'ip': ip_obj.network,
                                        'mask': ip_obj.netmask}
        return acl, ip_obj

    def build_acl_port(self, port_type, port, enabled):
    ...
    def apply_policy(self, policy):
        ...
        for rule_id, rule in policy['rules'].items():
            ...
            # Fixme(padkrish) actions that is not deny or allow, throw error
            ...
            acl = acl_str % {'tenant': tenant_name, 'action': action,
                             'prot': protocol}
            ip_acl, src_ip = self.build_acl_ip(rule, 'src')
            acl += ip_acl

            acl += self.build_acl_port('src', src_port, enabled)

            ip_acl, dst_ip = self.build_acl_ip(rule, 'dst')
            acl += ip_acl

            acl += self.build_acl_port('dst', dst_port, enabled)

            LOG.info(_LI("rule[%(rule_id)s]: name=%(name)s enabled=%(enabled)s"
                         " protocol=%(protocol)s dport=%(dport)s "
                         "sport=%(sport)s dip=%(dport)s dmask %(dmask)s "
                         "sip=%(sip)s smask %(smask)s action=%(dip)s"),
                     {'rule_id': rule_id, 'name': name, 'enabled': enabled,
                      'protocol': protocol, 'dport': dst_port,
                      'sport': src_port, 'dip': dst_ip.network,
                      'dmask': dst_ip.netmask, 'sip': src_ip.network,
                      'smask': src_ip.netmask, 'action': action})
