    ...
    def apply_policy(self, policy):
        ...
        for rule_id, rule in policy['rules'].items():
            ...
            if (rule['source_ip_address'] is not None):
                src_ip = netaddr.IPNetwork(rule['source_ip_address'])
            else:
                src_ip = netaddr.IPNetwork('0.0.0.0/0')

            if (rule['destination_ip_address'] is not None):
                dst_ip = netaddr.IPNetwork(rule['destination_ip_address'])
            else:
                dst_ip = netaddr.IPNetwork('0.0.0.0/0')
            ...
            if (rule['source_ip_address'] is None):
                ...
                acl += "%(net)s %(mask)s " % {'net': src_ip.network,
                                              'mask': src_ip.netmask}
            if (src_port is not None):
                if (':' in src_port):
                    ...
                    acl += "eq %(port)s " % {'port': src_port}
            if (rule['destination_ip_address'] is None):
                acl += "any "
            else:
                acl += "%(ip)s %(mask)s " % {'ip': dst_ip.network,
                                             'mask': dst_ip.netmask}
