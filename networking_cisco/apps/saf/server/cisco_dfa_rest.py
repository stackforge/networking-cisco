...
class DFARESTClient(object):
    ...
    def get_dcnm_http_or_https(self):
        ...
        """
        ret = self._verify_protocol('https')
        if ret:
            return 'https'
        ret = self._verify_protocol('http')
        if ret:
            return 'http'
        sys.exit(_LE("ERROR: Both http and https test failed"))

    def _build_host_protocol(self, protocol, host):
        """This function builds the host aand protocol of the URL. """
        return ('%s://%s/' % (protocol, self._ip))

    def _build_url(self, host_protocol, url_remaining):
        """This function builds the URL from host, protocol and string. """
        return host_protocol + url_remaining
    ...
    def fill_urls(self, protocol):
        ...
        host_protocol = self._build_host_protocol(protocol, self._ip)
        self._create_network_url = self._build_url(
            host_protocol, 'rest/auto-config/organizations'
            '/%s/partitions/%s/networks')
        ...
        self._global_settings_url = self._build_url(
            host_protocol, 'rest/auto-config/settings')
        self._create_part_url = self._build_url(
            host_protocol, 'rest/auto-config/organizations/%s/partitions')
        self._update_part_url = self._build_url(
            host_protocol, 'rest/auto-config/organizations/%s/partitions/%s')
        self._del_org_url = self._build_url(
            host_protocol, 'rest/auto-config/organizations/%s')
        ...
        self._segmentid_ranges_url = self._build_url(
            host_protocol, 'rest/settings/segmentid-ranges')
        self._login_url = self._build_url(host_protocol, 'rest/logon')
        self._logout_url = self._build_url(host_protocol, 'rest/logout')
