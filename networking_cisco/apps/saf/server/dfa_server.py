    ...
    def __init__(self, cfg):
        ...
        try:
            ...
            if segid_range is None:
                ...
                conf_min = int(
                    segid_range["segmentIdRanges"].partition("-")[0])
                conf_max = int(
                    segid_range["segmentIdRanges"].partition("-")[2])
                # The above lines can be minimized to:
                # conf_min, _, conf_max =
                # segid_range["segmentIdRanges"].partition("-")
                # But, doing so breaks the UT cases of test_dfa_server since
                # it mocks the whole module.
                # TODO(padkrish), Revisit this.
