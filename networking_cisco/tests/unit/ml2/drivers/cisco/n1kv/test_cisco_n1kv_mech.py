...
class TestN1KVMechDriverPortsV2(test_db_base_plugin_v2.TestPortsV2,
    ...
    def test_create_port_with_default_n1kv_policy_profile_id(
        ...
        with self.port() as port:
            ...
            port_prof_bindings = self.list_resource(
                resource='policy_profile_bindings',
                tenant_id=port_tenant)
            self.assertIn(
                needle=port_tenant,
                haystack=[d["tenant_id"] for d in port_prof_bindings[
                          "policy_profile_bindings"] if d["profile_id"] ==
                          pp['id']])

    def test_create_port_with_default_n1kv_policy_profile_id_restricted(self):
        """
        Test port creation with default policy profile, with both
        restricted and unrestricted access to policy profiles.
    ...
    def test_create_port_non_default_profile_restricted(self):
        ...
        with self.network() as network:
            res = self._create_port(
                ...
                set_context=True,
                arg_list=('n1kv:profile',),
                **{n1kv_const.N1KV_PROFILE: TEST_PP})
