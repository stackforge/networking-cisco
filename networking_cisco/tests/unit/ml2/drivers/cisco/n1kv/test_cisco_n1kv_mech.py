...
class TestN1KVMechanismDriver(
    ...
    def setUp(self):
        ...
        # Normal mock for most test cases- verifies request parameters.
            ...
            client_patch = mock.patch(n1kv_client.__name__ + ".Client",
                new=fake_client.TestClientGetDefaultppAsPortProfile)
        client_patch.start()
...
class TestN1KVMechDriverNetworksV2(test_db_base_plugin_v2.TestNetworksV2,
    ...
    def test_create_network_with_default_n1kv_vlan_network_profile_id(
        ...
        # assert that DB binding for the tenant is absent until he creates the
        # network with default VLAN network profile
        ...
        with self.network() as network:
            ...
            # assert that DB binding is created for the tenant who created the
            # network
            db_binding = n1kv_db.get_profile_binding(network_tenant_id, np.id)
            self.assertEqual(np.id, db_binding['profile_id'])
            self.assertEqual(network_tenant_id, db_binding['tenant_id'])

            # assert that API bindings also have tenant association with
            # the profile
            netprof_bindings = self.list_resource(
                resource='network_profile_bindings',
    ...
    def test_create_network_with_default_n1kv_vxlan_network_profile_id(
        ...
        # assert that DB binding for the tenant is absent until he creates the
        # network with default VxLAN network profile
        self.assertRaises(n1kv_exc.ProfileTenantBindingNotFound,
                          n1kv_db.get_profile_binding,
                          self._tenant_id,
                          np.id)
    ...
    def test_create_net_admin_defined_vlan_net_profile_name_unrestricted(self):
        ...
        for tenant_id in tenant_id_list:
            # assert that API binding of this net-profile with ANY tenant
...
class TestN1KVMechDriverPortsV2(test_db_base_plugin_v2.TestPortsV2,
    ...
    def test_create_port_with_default_n1kv_policy_profile_id(
        ...
        ml2_config.cfg.CONF.set_override(
            ...
            n1kv_const.DEFAULT_POLICY_PROFILE_NAME,
            'ml2_cisco_n1kv')
        ml2_config.cfg.CONF.set_override(
        ...
        with self.port() as port:
            ...
            # assert that binding for default-pp exists
            port_tenant = port['port']['tenant_id']
            netprof_bindings = self.list_resource(
                resource='policy_profile_bindings',
                tenant_id=port_tenant)
            self.assertIn(
                needle=port_tenant,
                haystack=[d["tenant_id"] for d in netprof_bindings[
                          "policy_profile_bindings"] if d["profile_id"] ==
                          pp['id']])
