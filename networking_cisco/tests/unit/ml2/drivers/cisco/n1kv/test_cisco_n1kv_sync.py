...
class TestN1kvSyncDriver(TestN1KVMechanismDriver):
    ...
    def setUp(self):
        ...
        # fake n1kv_client.Client.list_md5_hashes() for getting VSM MD5 hashes
    ...
    def _fake_get_neutron_res(self, res):
        """
        Mock function replacing SyncDriver._get_neutron_resource() that
        would be called from SyncDriver._md5_hash_comparison() function
    ...
    def _fake_get_vsm_md5_hashes(self, *args):
        ...
        that would be eventually called from
        SyncDriver._md5_hash_comparison() function
        :param args: Parameters for the original function. Have no use within
        this mocked function
        :return: Dictionary with all md5 hashes (including consolidated
        md5) as would have been returned by VSM
        """
    ...
    def test_md5_hash_comparison(self):
        """
        Test whether sync would be triggered for three test cases:
        i)   when neutron-VSM have identical resources
        ii)  when neutron has an extra network
        iii) when neutron has an extra port
        """

        # test with identical VSM-neutron configurations
        self.sync_driver._md5_hash_comparison(None)
        self.assertEqual(False, any(self.sync_driver.sync_resource.values()))

        # test network mismatch
        self.TEST_NEUTRON_NETWORKS.append('thisisanextranetworkonneutron')
        self.sync_driver._md5_hash_comparison(None)
        self.assertEqual(True, self.sync_driver.sync_resource[
            n1kv_const.NETWORKS])

        # test port mismatch
        self.TEST_NEUTRON_PORTS.append('thisisanextraportonneutron')
        self.sync_driver._md5_hash_comparison(None)
        self.assertEqual(True, self.sync_driver.sync_resource[
            n1kv_const.PORTS])
