...
class TestCiscoNexusVpcAllocDbTest(testlib_api.SqlTestCase):
    ...
    def setUp(self):
        original_intersect = nexus_db_v2._get_all_switch_vpcid_join

        def new_get_all_switch_vpcid_join(nexus_ips):
            intersect = list(original_intersect(nexus_ips))
            intersect.sort()
            return intersect

        mock.patch.object(nexus_db_v2,
                         '_get_all_switch_vpcid_join',
                         new=new_get_all_switch_vpcid_join).start()
    ...
    def test_vpcalloc_test_alloc_collision(self):
        ...
        def new_get_all_switch_vpcid_join(nexus_ips):
            results = nexus_db_v2.get_free_switch_vpcids('4.4.4.4')
            return results
