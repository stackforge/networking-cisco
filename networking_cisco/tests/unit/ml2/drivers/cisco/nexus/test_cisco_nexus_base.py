...
NEXUS_RESTAPI_DRIVER = ('networking_cisco.plugins.ml2.drivers.cisco.nexus.'
...
class TestCiscoNexusBase(testlib_api.SqlTestCase):
    ...
    def setUp(self):
        ...
        if cfg.CONF.ml2_cisco.use_restapi_driver:
            mock.patch.object(
                nexus_restapi_network_driver.CiscoNexusRestapiDriver,
                '_import_client',
        ...
        def new_nexus_init(mech_instance):
            if cfg.CONF.ml2_cisco.use_restapi_driver:
                mech_instance.driver = importutils.import_object(
                    NEXUS_RESTAPI_DRIVER)
            else:
                mech_instance.driver = importutils.import_object(
                    NEXUS_DRIVER)
