...
class TestCiscoUcsmMechDriver(testlib_api.SqlTestCase,
    ...
    def test_parse_ucsm_host_config(self):
        ...
        service_profile = ucsm_sp_dict.get(key)
        self.assertEqual(service_profile, 'SP1')
        self.assertNotEqual(service_profile, 'SP2')

        key = (cfg.CONF.ml2_cisco_ucsm.ucsm_ip, 'Host3')
        service_profile = ucsm_sp_dict.get(key)
        self.assertIsNone(service_profile)

        ucsm_ip = ucsm_host_dict.get('Host1')
        self.assertEqual(ucsm_ip, cfg.CONF.ml2_cisco_ucsm.ucsm_ip)

        ucsm_ip = ucsm_host_dict.get('Host3')
        self.assertIsNone(ucsm_ip, cfg.CONF.ml2_cisco_ucsm.ucsm_ip)
