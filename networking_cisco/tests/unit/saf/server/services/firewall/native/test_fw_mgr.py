...
class FwMgrTest(base.BaseTestCase):
    ...
    def setUp(self):
        ...
        with mock.patch('networking_cisco.apps.saf.server.services.firewall.'
                        'native.fabric_setup_base.FabricBase'),\
            mock.patch('networking_cisco.apps.saf.server.dfa_openstack_helper.'
                       'DfaNeutronHelper') as OsH:
            fw_mgr.FwMgr.events = mock.Mock()
            self.fw_mgr = fw_mgr.FwMgr(self.cfg)
            # Assert that events.update is called with right params and
            # OsHelper is called.
        OsH.assert_called_with()
    ...
    def _init_values(self):
        ...
        self.rule_dict['source_port'] = SRC_PORT
        self.rule_dict['destination_port'] = DST_PORT
        self.rule_dict['action'] = 'allow'

    def _fill_cfg(self):
        config.default_firewall_opts['firewall']['device'] = 'phy_asa'
        config.default_firewall_opts['firewall']['fw_mgmt_ip'] = '1.1.1.1'
        config.default_firewall_opts['firewall']['fw_username'] = 'user'
        config.default_firewall_opts['firewall']['fw_passport'] = 'user'
        config.default_firewall_opts['firewall']['fw_interface_in'] = 'e1/1'
        config.default_firewall_opts['firewall']['fw_interface_out'] = 'e1/1'
        return config
    ...
    def _fill_rule_data(self):
        fw_rule_dict = {}
        fw_rule_dict['firewall_rule'] = dict()
        fw_rule_dict['firewall_rule']['tenant_id'] = self.tenant_id
        sub_rule_dict = fw_rule_dict['firewall_rule']
        sub_rule_dict['firewall_policy_id'] = self.policy_id
        sub_rule_dict['id'] = self.rule_id
        sub_rule_dict['protocol'] = self.rule_dict['protocol']
        sub_rule_dict['source_ip_address'] = self.rule_dict[
            'source_ip_address']
        sub_rule_dict['destination_ip_address'] = self.rule_dict[
            'destination_ip_address']
        sub_rule_dict['source_port'] = self.rule_dict['source_port']
        sub_rule_dict['destination_port'] = self.rule_dict['destination_port']
        sub_rule_dict['action'] = self.rule_dict['action']
        sub_rule_dict['enabled'] = True
        sub_rule_dict['name'] = RULE_NAME
        return fw_rule_dict
    ...
    def _test_fw_rule_create(self):
        with mock.patch.object(self.fw_mgr.fabric,
            ...
            mock.patch.object(self.fw_mgr,
                              'update_fw_db_final_result') as update_fw_db,\
            mock.patch.object(self.fw_mgr,
                              'create_fw_device') as create_fw_dev,\
            mock.patch.object(self.fw_mgr,
                              'update_fw_db_dev_status') as update_fw_dev:
            ...
            parent.attach_mock(update_fw_db, 'update_fw_db_final_result')
            parent.attach_mock(update_fw_dev, 'update_fw_db_dev_status')
            prep_fab.assert_not_called()
            update_fw_db.assert_not_called()
            create_fw_dev.assert_not_called()
            update_fw_dev.assert_not_called()
    ...
    def _fill_policy_data(self, rule_dict):
        fw_policy_dict = {}
        fw_policy_dict['firewall_policy'] = dict()
        fw_policy_dict['firewall_policy']['tenant_id'] = self.tenant_id
        sub_policy_dict = fw_policy_dict['firewall_policy']
        sub_policy_dict['id'] = self.policy_id
        sub_policy_dict['name'] = 'PolicyA'
        sub_policy_dict['firewall_rules'] = rule_dict
    ...
    def _fill_fw_data(self, rule_dict):
        fw_dict = {}
        fw_dict['firewall'] = dict()
        fw_dict['firewall']['tenant_id'] = self.tenant_id
        sub_fw_dict = fw_dict['firewall']
        sub_fw_dict['id'] = self.fw_id
        sub_fw_dict['firewall_policy_id'] = self.policy_id
        sub_fw_dict['admin_state_up'] = True
        sub_fw_dict['name'] = self.fw_name
        sub_fw_dict['firewall_rules'] = rule_dict
        sub_fw_dict['router_ids'] = [self.router_id]
        # Fill the above with Policy rules by calling the fill rule fn
        return fw_dict
    ...
    def _prepare_result_fw_dict(self):
        ...
        fw_dict['firewall_policy_id'] = self.policy_id
        fw_dict['fw_type'] = self.fw_type
        fw_dict['router_id'] = self.router_id
        fw_dict['rules'][self.rule_id] = self.rule_dict
        return fw_dict
