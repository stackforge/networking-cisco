...
class PhyAsaTest(base_test.BaseTestCase):
    ...
    def _test_build_acl_ip(self, rule, type_ip):
        return self.asa_rest.build_acl_ip(rule, type_ip)

    def test_build_acl_ip_src(self):
        rule = {'source_ip_address': self.in_fabric_gw}
        acl, ip_obj = self._test_build_acl_ip(rule, 'src')
        self.assertEqual(acl, self.in_fabric_gw + ' 255.255.255.255 ')

    def test_build_acl_ip_dst(self):
        rule = {'destination_ip_address': self.out_fabric_gw}
        acl, ip_obj = self._test_build_acl_ip(rule, 'dst')
        self.assertEqual(acl, self.out_fabric_gw + ' 255.255.255.255 ')

    def test_build_acl_ip_no_key(self):
        acl, ip_obj = self._test_build_acl_ip({}, 'dst')
        self.assertEqual(acl, "any ")

    def test_build_acl_ip_none(self):
        acl, ip_obj = self._test_build_acl_ip({'destination_ip_address': None},
                                              'dst')
        self.assertEqual(acl, "any ")

    def _test_build_acl_port(self, port_type, port, enabled):
        return self.asa_rest.build_acl_port(port_type, port, enabled)

    def test_build_acl_port_src(self):
        acl = self._test_build_acl_port('src', str(SRC_PORT), True)
        self.assertEqual(acl, 'eq ' + str(SRC_PORT) + ' ')

    def test_build_acl_port_dst(self):
        acl = self._test_build_acl_port('dst', str(DST_PORT), True)
        self.assertEqual(acl, 'eq ' + str(DST_PORT) + ' ')

    def test_build_acl_port_not_enabled_dst(self):
        acl = self._test_build_acl_port('dst', str(DST_PORT), False)
        self.assertEqual(acl, 'eq ' + str(DST_PORT) + ' inactive')

    def test_build_acl_port_range_dst(self):
        port_range = str(DST_PORT) + ' : ' + str(DST_PORT + 10)
        acl = self._test_build_acl_port('dst', port_range, True)
        self.assertEqual(acl, 'range ' + port_range.replace(':', ' ') + ' ')
