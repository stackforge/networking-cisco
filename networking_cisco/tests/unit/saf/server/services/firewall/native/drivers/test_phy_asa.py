...
class PhyAsaTest(base_test.BaseTestCase):
    ...
    def test_build_acl_valid_ip(self):
        acl, ip_obj = self.asa_rest.build_acl_ip(self.in_fabric_gw)
    ...
    def test_get_ip_address_valid(self):
        ...
        self.assertEqual(ip_address, self.out_fabric_gw)

    def test_get_ip_address_null(self):
        ip_address = self.asa_rest.get_ip_address(None)
        self.assertEqual(ip_address, '0.0.0.0/0')

    def test_build_acl_ip_none(self):
        acl, ip_obj = self.asa_rest.build_acl_ip('0.0.0.0/0')
        self.assertEqual(acl, "any ")

    def test_build_acl_port_src(self):
        acl = self.asa_rest.build_acl_port('src', str(SRC_PORT), enabled=True)
        self.assertEqual(acl, 'eq ' + str(SRC_PORT) + ' ')

    def test_build_acl_port_dst(self):
        acl = self.asa_rest.build_acl_port('dst', str(DST_PORT), enabled=True)
        self.assertEqual(acl, 'eq ' + str(DST_PORT) + ' ')

    def test_build_acl_port_not_enabled_dst(self):
        acl = self.asa_rest.build_acl_port('dst', str(DST_PORT), enabled=False)
    ...
    def test_build_acl_port_range_dst(self):
        ...
        acl = self.asa_rest.build_acl_port('dst', port_range, enabled=True)
