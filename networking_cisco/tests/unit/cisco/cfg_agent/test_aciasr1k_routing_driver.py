...
class ASR1kRoutingDriverAci(asr1ktest.ASR1kRoutingDriver):
    def _get_vrf_name(self, vrf_tag, vlan):
        ...
        vrf_string = str(vrf_tag.encode('ascii', 'ignore') +
            hex(vlan)[2:].encode('ascii', 'ignore'))
        vrf_tag = uuid.uuid3(uuid.NAMESPACE_DNS, vrf_string).hex
    ...
    def setUp(self):
        ...
        self.vrf_pid = int(hashlib.md5(
            self.vrf.strip('nrouter-')[:6].encode('ascii',
                'ignore')).hexdigest()[:4], 16)
        ...
        int_ports = [self.port]
        self.router[bc.constants.INTERFACE_KEY] = int_ports
    ...
    def test_remove_secondary_ips_fail(self):
        self.assertIsNone(self.driver._remove_secondary_ips(None, {}))

    def test_get_interface_ip_key_error(self):
        fake_router = {'id': FAKE_ID}
        fake_port = {
            'hosting_info': {}
        }
        ri = routing_svc_helper.RouterInfo(FAKE_ID, fake_router)
        self.assertRaises(cfg_exc.DriverExpectedKeyNotSetException,
            self.driver._get_interface_ip_from_hosting_port,
            ri, fake_port)

    def test_get_interface_gateway_ip_key_error(self):
        fake_router = {'id': FAKE_ID}

        fake_port = {'hosting_info': {}}
        ri = routing_svc_helper.RouterInfo(FAKE_ID, fake_router)
        self.assertRaises(cfg_exc.DriverExpectedKeyNotSetException,
            self.driver._get_interface_gateway_ip_from_hosting_port,
            ri, fake_port)

    def test_get_interface_subnet_key_error(self):
        fake_router = {'id': FAKE_ID}

        fake_port = {'hosting_info': {}}
        ri = routing_svc_helper.RouterInfo(FAKE_ID, fake_router)
        self.assertRaises(cfg_exc.DriverExpectedKeyNotSetException,
            self.driver._get_interface_subnet_from_hosting_port,
            ri, fake_port)
    ...
    def test_internal_network_added_with_multi_region(self):
        ...
        is_multi_region_enabled = cfg.CONF.multi_region.enable_multi_region
        self.assertTrue(is_multi_region_enabled)
    ...
    def test_external_gateway_added_global_config(self):
        ...
        LOG.warn("mock calls are: %s" %
                 self.driver._ncc_connection.edit_config.mock_calls)
    ...
    def test_floating_ip_removed_extra_subnets(self):
        ...
        self.router[bc.constants.FLOATINGIP_KEY] = fips
        ...
        self.router[bc.constants.FLOATINGIP_KEY].pop()
        self.driver.floating_ip_removed(self.ri, self.ex_gw_port,
                                        self.floating_ip, self.fixed_ip)
