...
from networking_cisco.plugins.cisco.l3.drivers.asr1k import (
    aci_asr1k_routertype_driver as aci_driver)
...
BC = 'networking_cisco.backwards_compatibility.get_plugin'
...
class AciAsr1kHARouterTypeDriverTestCase(
    ...
    def test_update_floating_ip_pre_and_post(self):
        with self.subnet() as ext_s, \
                self.subnet(cidr='10.0.1.0/24') as sub1, \
                self.subnet(cidr='20.0.1.0/24') as sub2:
            s = ext_s['subnet']
            ext_net_id = s['network_id']
            self._set_net_external(ext_net_id)
            with self.router(
                    external_gateway_info={'network_id': ext_net_id}) as r1,\
                    self.router(
                    external_gateway_info={'network_id': ext_net_id}) as r2,\
                    self.port(sub1) as p1, self.port(sub2) as p2:
                self._router_interface_action('add', r1['router']['id'], None,
                                              p1['port']['id'])
                self._router_interface_action('add', r2['router']['id'], None,
                                              p2['port']['id'])
                p1 = p1['port']
                p2 = p2['port']
                ...
                #floating_ip['router_id'] = r2['router']['id']
                fip = {'floatingip': {'port_id': p2['id'],
                                      'tenant_id': floating_ip['tenant_id']}}
                self.l3_plugin.update_floatingip(ctx, floating_ip['id'], fip)
                # Unfortunately there's a difference between upstream
                # neutron's handling of the fip context -- it appends
                # the ID to what's passed in for earlier releases, which
                # means we can't validate the third argument. Use ANY
                # instead
                self.drv.update_floatingip_precommit.assert_called_once_with(
                    ctx, floating_ip['id'], mock.ANY)
                self.drv.update_floatingip_postcommit.assert_called_once_with(
                    ctx, floating_ip['id'], mock.ANY)
...
class AciAsr1kRouterTypeDriverNeutronTestCase(
    ...
    def setUp(self):
        ...
        self.dummy_apic_ml2_l3 = mock.Mock()
    ...
    def test_get_apic_driver(self):
        ...
        with self.router(tenant_id=tenant_id) as router1:
            self.l3_plugin._process_backlogged_routers()
            r1 = router1['router']
            router_type_id = self.l3_plugin.get_router_type_id(ctx, r1['id'])
            driver = self.l3_plugin._get_router_type_driver(context,
                router_type_id)
            with mock.patch('oslo_utils.importutils.import_object',
                            return_value=self.dummy_apic_ml2_l3):
                self.assertEqual(self.dummy_apic_ml2_l3, driver.apic_driver)

    def test_get_apic_driver_key_error(self):
        tenant_id = _uuid()
        ctx = context.Context('', '', is_admin=True)
        with self.router(tenant_id=tenant_id) as router1:
            self.l3_plugin._process_backlogged_routers()
            ...
            try:
                mydriver = driver.apic_driver  # noqa
                self.fail("Test didn't raise "
                    "PluggingDriverNoAciDriverInstalledOrConfigured")
            except aci_driver.PluggingDriverNoAciDriverInstalledOrConfigured:
                pass
            except Exception:
                self.fail("Test didn't raise "
                    "PluggingDriverNoAciDriverInstalledOrConfigured")

    def test_get_apic_driver_no_driver(self):
        tenant_id = _uuid()
        ctx = context.Context('', '', is_admin=True)
        with self.router(tenant_id=tenant_id) as router1:
            self.l3_plugin._process_backlogged_routers()
...
class AciAsr1kRouterTypeDriverGbpTestCase(
        cisco_test_case.L3RoutertypeAwareHostingDeviceSchedulerTestCaseBase):

    router_type = 'AciASR1k_Neutron_router'

    def setUp(self):
        super(AciAsr1kRouterTypeDriverGbpTestCase, self).setUp()
        mock_gbp_l3 = mock.Mock()
        policy_driver = mock.MagicMock(return_value=object())
        mock_gbp_l3.policy_driver_manager.policy_drivers = policy_driver
        self.dummy_gbp_l3 = mock_gbp_l3
    ...
    def test_get_apic_driver(self):
        ...
        with self.router(tenant_id=tenant_id) as router1:
            ...
            driver = self.l3_plugin._get_router_type_driver(context,
                router_type_id)
            with mock.patch(BC, return_value=self.dummy_gbp_l3):
                with mock.patch('oslo_utils.importutils.import_object',
    ...
    def test_get_apic_driver_key_error(self):
        ...
        with self.router(tenant_id=tenant_id) as router1:
            ...
            driver = self.l3_plugin._get_router_type_driver(context,
                router_type_id)
            self.dummy_gbp_l3.policy_driver_manager.policy_drivers = {}
            with mock.patch(BC, return_value=self.dummy_gbp_l3):
                try:
                    mydriver = driver.apic_driver  # noqa
                    self.fail("Test didn't raise KeyError")
                except KeyError:
                    pass
                except Exception:
                    self.fail("Test didn't raise KeyError")

    def test_get_apic_driver_import_fail(self):
        tenant_id = _uuid()
        ctx = context.Context('', '', is_admin=True)
        with self.router(tenant_id=tenant_id) as router1:
            self.l3_plugin._process_backlogged_routers()
            ...
            with mock.patch(BC, return_value=self.dummy_gbp_l3):
                try:
                    mydriver = driver.apic_driver  # noqa
                    self.fail("Test didn't raise Exception")
                except Exception:
                    pass
