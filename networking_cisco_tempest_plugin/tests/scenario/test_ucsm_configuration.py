...
class TestUCSMConfiguration(base.BaseAdminNetworkTest):
    ...
    def test_ucsm_config_scenario(self):
        ...
        network = self.admin_networks_client.show_network(network['id'])
        network_vlan = network['provider:segmentation_id']

        # Connect to UCSM
        handle = UcsHandle(CONF.ucsm.ucsm_up, CONF.ucsm.ucsm_username,
                CONF.ucsm.ucsm_password)
        handle.login()

        vlan = handle.query_dn('fabric/lan/net-OS-PP-' + network_vlan)
        if vlan is not None:
            self.assertEqual(network_vlan, vlan.id)
        else:
            raise Exception('Port Profile not created on UCSM: %s',
                             network_vlan)
